...
public class TextureHelper {
    ...
    public static int loadTexture(Context context, int resourceId){
        ...
        if(bitmap == null){
            if(LoggerConfig.ON){
                Log.w(TAG, "ResourceID " + resourceId + " couldnt be decoded");
            }
    ...
    public static int loadCubeMap(Context context, int[] cubeResources) {
        ...
        if(textureObjectsIds[0] == 0) {
            if(LoggerConfig.ON) {
                Log.w(TAG, "Couldn't generate a new OpneGL texture object");
            }
            return 0;
        }
        ...
        for(int i = 0; i < 6; i++) {
            ...
            if(cubeBitmaps[i] == null) {
                if(LoggerConfig.ON) {
                    Log.w(TAG, "Resource ID" + cubeResources[i] + " couldn't be decoded");
                }
        ...
        glBindTexture(GL_TEXTURE_CUBE_MAP, textureObjectsIds[0]);
